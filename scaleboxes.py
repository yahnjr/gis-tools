...
layout = aprx.listLayouts(arcpy.GetParameterAsText(0))[
    0
]  # Assuming there's only one layout named "Name"
...
def scale_boxes(value1, value2, value3):
    ...
    print(
        "A " + str(value1) + " acre square would be " + str(five_acre_side) + " inches"
    )

    print(
        "A "
        + str(value2)
        + " acre square would be "
        + str(twenty_acre_side)
        + " inches"
    )

    print(
        "A " + str(value3) + " acre square would be " + str(fifty_acre_side) + " inches"
    )
